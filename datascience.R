...
suppressWarnings(suppressMessages(install_packages(packages)))

#' install.packages(c("devtools", "roxygen2"))
#' setwd(parentDirectory)
#' create("easyDataScience")
#' setwd("./domaghale/easyDataScience")
#' document()
#' setwd("..")
...
#' write column names as a vector for an example c("col1", "col2")
unique_keys_more <- function(data, ...) {
	distinct_(data[,...]) #' print as column format
}

#' We must impute the missing data in a variable by either replacing it with the mean/median value of all the available variable or with 0, or removing the row
